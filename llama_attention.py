...
print('Output (no mask):', np.array(np.nditer(out_no_mask.detach().numpy(), order='F')))

attention.config._attn_implementation = 'sdpa'
out, _ = attention.forward(x, position_embeddings=(cos, sin), attention_mask=None)
print('Output (causal):', np.array(np.nditer(out.detach().numpy(), order='F')))
